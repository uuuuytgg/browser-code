...
const envPath = process.env.BROWSER_CODE_BIN_PATH
const scriptDir = path.dirname(fs.realpathSync(__filename))
const cached = path.join(scriptDir, ".browser-code")
const platform = { darwin: "darwin", linux: "linux", win32: "windows" }[os.platform()] || os.platform()
const arch = { x64: "x64", arm64: "arm64", arm: "arm" }[os.arch()] || os.arch()
const base = "@opencode-ai/cli-" + platform + "-" + arch
const binary = platform === "windows" ? "browser-code.exe" : "browser-code"
...
if (!resolved) {
  console.error(
    "It seems that your package manager failed to install the right browser-code CLI package. Try manually installing " +
