...
export function cliErrorMessage(input: unknown): string | undefined {
  ...
  if (model) {
    ...
    return [
      ...
      "Try: `browser-code models` to list available models",
      "Or check your config (browser-code.json) provider/model names",
    ].join("\n")
  }
  ...
  if (isRecord(input) && named(input, "MCPFailed")) {
    ...
    return `MCP server "${name}" failed. Note, browser-code does not support MCP authentication yet.`
  }
  return undefined
}
