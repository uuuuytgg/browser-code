...
function globalConfigFile() {
  const candidates = ["browser-code.jsonc", "browser-code.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.BROWSER_CODE_CONFIG && !Flag.BROWSER_CODE_CONFIG_DIR && !Flag.BROWSER_CODE_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "browser-code.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "browser-code.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "BROWSER_CODE_CONFIG_CONTENT") return "local"
        ...
        if (Flag.BROWSER_CODE_CONFIG) {
          yield* merge(Flag.BROWSER_CODE_CONFIG, yield* loadFile(Flag.BROWSER_CODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.BROWSER_CODE_CONFIG })
        }

        if (!Flag.BROWSER_CODE_DISABLE_PROJECT_CONFIG) {
        ...
        if (Flag.BROWSER_CODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from BROWSER_CODE_CONFIG_DIR", { path: Flag.BROWSER_CODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".browser-code") || dir === Flag.BROWSER_CODE_CONFIG_DIR) {
            for (const file of ["browser-code.json", "browser-code.jsonc"]) {
        ...
        if (process.env.BROWSER_CODE_CONFIG_CONTENT) {
          const source = "BROWSER_CODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.BROWSER_CODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from BROWSER_CODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["BROWSER_CODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("BROWSER_CODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.BROWSER_CODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.BROWSER_CODE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("BROWSER_CODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.BROWSER_CODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.BROWSER_CODE_DISABLE_PRUNE) {
