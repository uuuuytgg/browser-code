...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "browser-code"),
    ...(await Filesystem.findUp(["browser-code.json", "browser-code.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "opencode"))
  }
  if (Flag.BROWSER_CODE_CONFIG) files.push(Flag.BROWSER_CODE_CONFIG)
