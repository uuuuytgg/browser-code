...
declare global {
  const BROWSER_CODE_WORKER_PATH: string
}
...
async function target() {
  if (typeof BROWSER_CODE_WORKER_PATH !== "undefined") return BROWSER_CODE_WORKER_PATH
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start browser-code tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start browser-code in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const transport = external
        ...
        : {
            url: "http://browser-code.internal",
