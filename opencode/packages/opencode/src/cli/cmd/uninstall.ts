...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g browser-code-ai",
      pnpm: "pnpm uninstall -g browser-code-ai",
      bun: "bun remove -g browser-code-ai",
      yarn: "yarn global remove browser-code-ai",
      brew: "brew uninstall browser-code",
      choco: "choco uninstall browser-code",
      scoop: "scoop uninstall browser-code",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".browser-code")) {
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# browser-code") || content.includes(".browser-code/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".browser-code/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".browser-code/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".browser-code"))
    ) {
      continue
    }
