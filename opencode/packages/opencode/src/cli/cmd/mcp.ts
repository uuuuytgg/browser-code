...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in browser-code.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  ...
  const candidates = [path.join(baseDir, "browser-code.json"), path.join(baseDir, "browser-code.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".browser-code", "browser-code.json"), path.join(baseDir, ".browser-code", "browser-code.jsonc"))
  }
