...
async function registerCurrentSession(ctx: BootContext, sessionID: string): Promise<void> {
  CurrentSession.set({ sessionID: sessionID as SessionID })

  const client = (ctx.sdk as unknown as { client?: RawSdkClient }).client
  if (!client?.put || !sessionID) {
    return
  }

  await client.put({
    url: "/tui/session/current",
...
async function runInteractiveRuntime(input: RunRuntimeInput, deps: RunRuntimeDeps = {}): Promise<void> {
  ...
  const ensureSession = () => {
    if (!input.resolveSession || state.sessionID) {
      void registerCurrentSession(ctx, state.sessionID).catch((error) => {
        log?.write("session.current.failed", {
          sessionID: state.sessionID,
    ...
    state.session = input.resolveSession(ctx).then((next) => {
      ...
      return registerCurrentSession(ctx, state.sessionID).catch((error) => {
        log?.write("session.current.failed", {
          sessionID: state.sessionID,
          error: error instanceof Error ? error.message : String(error),
        })
  ...
  if (Flag.BROWSER_CODE_SHOW_TTFD) {
    footer.append({
      kind: "system",
