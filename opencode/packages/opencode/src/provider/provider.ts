    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>, model?: Model) {
          ...
          // 1. options.region from browser-code.json provider config
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        // load env — fall back to process.env when Env.Service is unavailable
        // (e.g. in serve mode without InstanceContext)
        const envs: Record<string, string | undefined> = yield* env.all().pipe(
          Effect.catchCause(() =>
            Effect.succeed(
              Object.fromEntries(
