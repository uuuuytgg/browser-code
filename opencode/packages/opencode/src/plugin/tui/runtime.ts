...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".browser-code"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".browser-code", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".browser-code", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.BROWSER_CODE_PURE ? [] : pluginOrigins
    if (Flag.BROWSER_CODE_PURE && pluginOrigins.length) {
    }
