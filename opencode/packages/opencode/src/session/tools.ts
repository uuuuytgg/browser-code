...
export const resolve = Effect.fn("SessionTools.resolve")(function* (input: {
  ...
  for (const item of yield* registry.tools({
    ...
    if (!allowToolForBrowserCodeCoreContext(item.id, input.browserCodeCoreContext)) continue
  ...
  if (
    hasMcpResourceServer
    && allowMcpToolForBrowserCodeCoreContext(MCP_RESOURCE_TOOLS.list, input.browserCodeCoreContext)
  ) {
    ...
    if (allowMcpToolForBrowserCodeCoreContext(MCP_RESOURCE_TOOLS.listTemplates, input.browserCodeCoreContext)) {
    ...
    if (allowMcpToolForBrowserCodeCoreContext(MCP_RESOURCE_TOOLS.read, input.browserCodeCoreContext)) {
  ...
  for (const [key, item] of Object.entries(yield* mcp.tools())) {
    if (!allowMcpToolForBrowserCodeCoreContext(key, input.browserCodeCoreContext)) continue
