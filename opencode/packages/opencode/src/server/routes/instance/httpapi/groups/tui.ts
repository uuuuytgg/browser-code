...
export const TuiCurrentSessionPayload = Schema.Struct({
  sessionID: SessionID,
})
...
export const TuiPaths = {
  ...
  currentSession: `${root}/session/current`,
  ...
  .add(
    HttpApiGroup.make("tui")
      .add(
        ...
        HttpApiEndpoint.get("currentSession", TuiPaths.currentSession, {
          query: WorkspaceRoutingQuery,
          success: described(CurrentSessionInfo, "Current TUI session"),
          error: ApiNotFoundError,
        }).annotateMerge(
          OpenApi.annotations({
            identifier: "tui.session.current",
            summary: "Get current TUI session",
            description: "Return the session currently controlled by the interactive TUI.",
          }),
        ),
        HttpApiEndpoint.put("setCurrentSession", TuiPaths.currentSession, {
          query: WorkspaceRoutingQuery,
          payload: TuiCurrentSessionPayload,
          success: described(CurrentSessionInfo, "Current TUI session"),
          error: [HttpApiError.BadRequest, ApiNotFoundError],
        }).annotateMerge(
          OpenApi.annotations({
            identifier: "tui.session.setCurrent",
