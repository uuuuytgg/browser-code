...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    const current = Effect.fn("SessionHttpApi.current")(function* () {
      const info = CurrentSession.get()
      if (!info) return yield* Effect.fail(notFound("Current TUI session is not registered"))
      yield* requireSession(info.sessionID)
      return info
    })

    const setCurrent = Effect.fn("SessionHttpApi.setCurrent")(function* (ctx: {
      payload: typeof CurrentSessionPayload.Type
    }) {
      yield* requireSession(ctx.payload.sessionID)
      return CurrentSession.set({ sessionID: ctx.payload.sessionID })
    })
      ...
      yield* promptSvc.prompt({ ...ctx.payload, sessionID: ctx.params.sessionID }).pipe(
        Effect.catchCause((cause) =>
          Effect.gen(function* () {
            ...
            yield* eventBridge.publish(Session.Event.Error, {
              sessionID: ctx.params.sessionID,
    ...
    const events = Effect.fn("SessionHttpApi.events")(function* (ctx: {
      params: { sessionID: SessionID }
      request: HttpServerRequest.HttpServerRequest
    }) {
      return yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* requireSession(targetSessionID)
          yield* Effect.logInfo("session events connected", { sessionID: targetSessionID })
          const queue = yield* Queue.unbounded<{ type: string; data: unknown }>()
          const unsubscribe = yield* eventBridge.listen((event) =>
            Effect.sync(() => {
              if (!event.data || typeof event.data !== "object") return
              const d = event.data as Record<string, unknown>
              if (d.sessionID === targetSessionID || d.sessionID === undefined) {
                Queue.offerUnsafe(queue, { type: event.type, data: event.data })
              }
            }),
          )
          ...
          const heartbeat = Stream.tick("10 seconds").pipe(
            Stream.drop(1),
            Stream.map((): { type: string; data: unknown } => ({ type: "server.heartbeat", data: {} })),
          )
          return HttpServerResponse.stream(
            Stream.make({ type: "server.connected", data: { sessionID: targetSessionID } } as const).pipe(
              Stream.concat(stream.pipe(Stream.merge(heartbeat, { haltStrategy: "left" }))),
              Stream.map((event): Sse.Event => ({
                _tag: "Event",
                event: "message",
                id: undefined,
              ...
              Stream.encodeText,
              Stream.ensuring(Effect.logInfo("session events disconnected", { sessionID: targetSessionID })),
            ),
            {
