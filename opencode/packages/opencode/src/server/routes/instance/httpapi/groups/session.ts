...
import { CurrentSessionInfo } from "@/session/current"
...
export const CurrentSessionPayload = Schema.Struct({
  sessionID: SessionID,
})
  ...
  .add(
    HttpApiGroup.make("session")
      .add(
        ...
        HttpApiEndpoint.get("current", SessionPaths.current, {
          query: WorkspaceRoutingQuery,
          success: described(CurrentSessionInfo, "Get current TUI session"),
          error: ApiNotFoundError,
        }).annotateMerge(
          OpenApi.annotations({
            identifier: "session.current",
            summary: "Get current TUI session",
            description: "Return the session currently controlled by the interactive TUI.",
          }),
        ),
        HttpApiEndpoint.put("setCurrent", SessionPaths.current, {
          query: WorkspaceRoutingQuery,
          payload: CurrentSessionPayload,
          success: described(CurrentSessionInfo, "Set current TUI session"),
          error: [HttpApiError.BadRequest, ApiNotFoundError],
        }).annotateMerge(
          OpenApi.annotations({
            identifier: "session.setCurrent",
            summary: "Set current TUI session",
            description: "Record the session currently controlled by the interactive TUI.",
          }),
        ),
        ...
        HttpApiEndpoint.get("events", SessionPaths.events, {
          params: { sessionID: SessionID },
          query: WorkspaceRoutingQuery,
