...
import { CommandPayload, TuiCurrentSessionPayload, TuiPublishPayload } from "../groups/tui"
...
export const tuiHandlers = HttpApiBuilder.group(InstanceHttpApi, "tui", (handlers) =>
  Effect.gen(function* () {
    ...
    const currentSession = Effect.fn("TuiHttpApi.currentSession")(function* () {
      const info = CurrentSession.get()
      if (info) {
        yield* SessionError.mapStorageNotFound(session.get(info.sessionID))
        return info
      }

      const latest = yield* session.list({ limit: 1 })
      const fallback = latest[0]
      if (!fallback) return yield* Effect.fail(notFound("Current TUI session is not registered"))
      return CurrentSession.set({ sessionID: fallback.id })
    })

    const setCurrentSession = Effect.fn("TuiHttpApi.setCurrentSession")(function* (ctx: {
      payload: typeof TuiCurrentSessionPayload.Type
    }) {
      yield* SessionError.mapStorageNotFound(session.get(ctx.payload.sessionID))
    ...
    return handlers
      ...
      .handle("currentSession", currentSession)
      .handle("setCurrentSession", setCurrentSession)
