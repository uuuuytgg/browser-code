...
| Skill blocking | Claude Code 说匹配 skill 是 blocking requirement | Browser Code 路由型 skill 会抢在 ProReader 前；执行后端型 skill 又可能是 plan 落地所必需 | `SystemPrompt.skills()` 在 ProReader preflight 阶段区分 skill 类型；`SessionTools.resolve()` 按 action 激活执行后端 | 反向借鉴：路由权归 ProReader，执行后端按 plan 开放 |
...
阶段 1: ProReader preflight
  ...
  不可见: 路由型 skill, task, websearch 发起权, platform search 发起权, aihot
  注意: multi-search-engine / SQL blooming 这类搜索执行后端不是首层意图入口；preflight 阶段不主动暴露，但必须能在 ProReader plan 选择 websearch 后被激活

阶段 2: ProReader route returned
  可见: ProReader plan 选中的 provider tools
  可见: 被选中 provider 所需的执行后端，例如 websearch -> multi-search-engine -> SQL/search backend
  不可见: 未选中的工具
...
- 如果照搬 Claude Code 的 skill blocking，`aihot` 这类强描述路由型 skill 会抢 ProReader。
- 但不能把所有 skill 都当成要禁止的入口：`multi-search-engine` 这类能力可能是 `websearch` 的执行后端，禁止它会导致 ProReader plan 无法落地。
...
非 URL 自然语言:
  ...
  路由型 skill 不可见或被提示为 post-route provider
  执行后端型 skill 不主动暴露为入口，但保留为 ProReader action 的可激活 backend

ProReader 判定为 trend/news/AI hot:
  aihot 才作为 provider/action 进入执行阶段

ProReader 判定需要 websearch:
  允许 websearch action 激活 multi-search-engine / SQL blooming 等执行后端
```
...
```text
路由型 skill:
  会决定任务方向或内容来源，例如 aihot、某些平台发现 skill。
  这类不能抢 ProReader first-turn。

执行后端型 skill / 工具包:
  不决定用户意图，只完成被选中 action 的底层执行，例如 multi-search-engine、SQL blooming、搜索聚合后端。
  这类不能作为 first-turn 入口，但必须能被 ProReader plan 激活。
```
...
- `opencode/packages/opencode/src/session/system.ts`
  ...
  - 应按 `BrowserCodeCoreContext.phase` 和 skill 类型控制。
- `opencode/packages/opencode/src/tool/skill.ts`
  - 当前 Skill tool 可执行任何 available skill。
  - 可增加 Browser Code phase 检查，preflight 阶段拒绝非显式路由型 skill。
- `opencode/packages/opencode/src/session/tools.ts`
  - 必须允许 ProReader action 激活所需执行后端，不能因为它由 skill/SQL 实现就拦截。
...
- 如果 ProReader 返回 `websearch` action 且 readiness 命中 `multi_search_engine`，必须允许 multi-search-engine 及其 SQL/search backend 执行。
...
不要让路由型 skill 继续在 first-turn 自选；执行后端型 skill / 工具包必须能被 ProReader action 激活。
...
这部分不是 ProReader 首路由 P0 的核心，但会影响研究结果如何入库，必须写进后续 review/save 阶段设计。

当前判断：

- 没找到可以直接复用的现成 Claude / Claude Code “研究候选入库选择器”实现。
- Claude 可借鉴的是 review gate、workflow opt-in、citation/evidence discipline 这些模式。
- Browser Code 需要自己实现轻量选择器，但不用一开始做复杂 HTML。

需要区分两种输出形态：

```text
报告型研究:
  ProReader 将多信源吸收、去重、归纳成单篇研究报告。
  保存时默认保存一个报告笔记。
  sources / citations / evidence refs 附在报告中。

候选型发现:
  ProReader 输出多条候选资料，例如视频、文章、论文、帖子、repo。
  保存时必须让用户选择哪些候选入库、哪些只作引用、哪些丢弃。
```
...
```text
review_selection 阶段
  输入: ProReader candidates / evidence list
  输出: 用户选择的 candidate ids
  保存策略:
    - save_report_only
    - save_selected_candidates
    - save_report_and_selected_candidates
```

最小交互先用 Markdown / 编号，不急着做 HTML：

```markdown
## 可保存候选

- [ ] C1 标题
  - 来源: Bilibili / GitHub / Wikipedia
  - 价值: 高
  - 建议: 单独入库 / 仅作引用 / 丢弃
...
```text
保存 C1,C4,C7，并把 C2 只作为引用
```

后续再考虑 HTML 选择器：

- HTML 适合候选很多、需要批量勾选时使用。
- Markdown 适合先把状态机跑通，避免 UI 复杂度阻塞 P0。

验收：

- 如果 ProReader 输出的是综合报告，默认不要求逐条选择。
- 如果 ProReader 输出的是多候选列表，不能只问“是否全部保存”。
- 用户必须能选择保存部分候选。
- 未选候选可以保留为报告引用，但不能默认全部入库。
...
禁止 first batch: aihot, bilibili_search, websearch 发起权, task
ProReader 判定后再选择 provider
如果选择 websearch，则允许 multi-search-engine / SQL backend 执行
```
